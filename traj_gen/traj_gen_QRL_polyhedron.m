...
for k = 1:N
    ...
    constr = [constr, xL<=x(:,k)<=xU, uL<=u(:,k)<=uU, ...
        ...
        %         0.01<=Topt(k)<=0.375...
        x(:,k+1)==x(:,k)+Topt*obj.systemDynamics([],x(:,k),u(:,k)), ...
        0.01<=Topt<=0.375...
        ];
...
figure
h1 = plot(traj.x(1,:),traj.x(2,:),'x-');
xlabel('y')
ylabel('z')
title('Generated trajectory with obstacle avoidance')
hold on
for m=1:M
    plot(O{m},'alpha',0.5)
end

h2 = plot(xinit(1,:),xinit(2,:),'r');

h3 = plot([xL(1) xL(1)],[xL(2) xU(2)],'k','LineWidth',2);
