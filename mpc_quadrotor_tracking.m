...
% -----
% act_sys.mQ = sys.mQ*1.2; % actual mass is different estimated mass
% act_sys.JQ = sys.JQ*1.5; % similarly J is also different
...
xref = zeros(sys.nDof,params.mpc.M+params.mpc.N);
uref = (sys.mQ*sys.g/2)*ones(2,params.mpc.M+params.mpc.N);

% waypoints
xref = [zeros(sys.nDof,floor(params.mpc.M/2)), repmat([0.5;0.5;0;0;0;0],1,params.mpc.M+params.mpc.N-floor(params.mpc.M/2))];
uref = (sys.mQ*sys.g/2)*ones(2,params.mpc.M+params.mpc.N);
